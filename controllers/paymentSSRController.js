...
const renderPayment = async (req, res) => {
  try {
    const { id } = req.params;
    const user_id = req.user._id;
    const payment = await Payment.findById(id).where('user_id').equals(user_id);
    if (!payment) {
...
const addPayment = async (req, res) => {
  try {
    ...
    const newPayment = new Payment({ amount, method, status, achieved,user_id });
...
const deletePayment = async (req, res) => {
  try {
    const { id } = req.params;
    const user_id = req.user._id;
    const payment = await Payment.findByIdAndDelete({ _id: id,user_id: user_id });
    if (!payment) {
...
const updatePayment = async (req, res) => {
  try {
    ...
    const achieved = req.body.achieved === "on";
    const { amount, method, status } = req.body;
    const updatedPaymentData = { amount, method, status, achieved };

    // Update the payment with the new data
    const updatedPayment = await Payment.findOneAndUpdate({ _id: id, user_id: user_id }, updatedPaymentData, { new: true });
